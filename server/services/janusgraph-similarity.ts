...
export class JanusGraphSimilarityService {
  ...
  private generateGraphBasedRecommendations(data: any): string[] {
    ...
    if (data.indirect > 50) {
      recommendations.push('Extensive indirect impact detected - coordinate with downstream teams');
    }
    
    if (data.systems && data.systems.length > 3) {
      recommendations.push(`Multi-system impact across ${data.systems.length} systems - plan cross-team coordination`);
    }
